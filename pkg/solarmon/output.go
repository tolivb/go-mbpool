...
const MaxInfluxQSize = 500
const MaxPostsPerFlush = 10
...
func (o *InfluxOutput) WriteRegisters(data []*Register) error {
	...
	if len(queries) == 0 {
		o.out.Write(fmt.Sprintf("ERR Influxdb: no queries to exec\n"))
		return nil
	}
	...
	for tries := 1; len(o.q) > 0 && tries <= MaxPostsPerFlush; tries++ {
		...
		if err != nil {
			...
			o.out.Write(fmt.Sprintf("ERR Influxdb: qsize=%d req=%d %s\n", len(o.q), tries, errTxt))
			break
		} else {
			o.q = o.q[1:]
			if startQSize > 1 {
				o.out.Write(fmt.Sprintf("OK Influxdb old record flush: qsize=%d req=%d\n", len(o.q), tries))
			}
		}
	}
	return nil
...
func (o *InfluxOutput) prepareQueries(registers []*Register) []string {
	...
	for _, register := range registers {
		...
		case "5m":
			min := t.Minute() - (t.Minute() % 5)
			ts = time.Date(t.Year(), t.Month(), t.Day(), t.Hour(), min, 0, 0, time.UTC)
		case "-1h":
			ts = time.Date(t.Year(), t.Month(), t.Day(), t.Hour()-1, 0, 0, 0, time.UTC)
		case "1h":
			ts = time.Date(t.Year(), t.Month(), t.Day(), t.Hour(), 0, 0, 0, time.UTC)
		case "-1d":
			ts = time.Date(t.Year(), t.Month(), t.Day()-1, 5, 0, 0, 0, time.UTC)
		case "1d":
			ts = time.Date(t.Year(), t.Month(), t.Day(), 5, 0, 0, 0, time.UTC)
		case "1m":
			ts = time.Date(t.Year(), t.Month(), 1, 5, 0, 0, 0, time.UTC)
		case "-1m":
			ts = time.Date(t.Year(), time.Month(int(t.Month())-1), 1, 5, 0, 0, 0, time.UTC)
		case "1y":
			ts = time.Date(t.Year(), time.Month(1), 1, 5, 0, 0, 0, time.UTC)
		case "-1y":
			ts = time.Date(t.Year()-1, time.Month(1), 1, 5, 0, 0, 0, time.UTC)
		case "inf":
			ts = time.Unix(0, 0)
		}
		...
		tsvalue := ""
		if register.TsType != "now" {
			tsvalue = fmt.Sprintf("%v", ts.UnixNano())
		} else {
			tsvalue = fmt.Sprintf("%v", time.Now().UnixNano())
		}
